...
var getAttributes = function (node) {
    if (typeof node == 'object' && node[0] == 1) {
        return node[3];
    }
    return null;
};
...
function canHaveChildren(node) {
    if (node) {
        ...
        return (type == "element" || type == "document");
    }
}
...
function copyWithValOnChildIndex(node, value) {
    if (node) {
        var type = nodeTypes[node[0]];
        if (type == "element") {
            if (node[3]) {
                return [node[0], node[1], value, node[3]];
            } else {
...
module.exports = {
    ...
    tokenize: function (tokenizer, content) {
        ...
        function recursivelyTokenize(node, section) {
            if (getTagName(node) == 'section') {

                section = getAttribute(node, 'role');
            }

            var tokenObjs = [];

            if (canHaveChildren(node)) {
                tokenObjs.push(copyWithValOnChildIndex(node, OPEN_TAG));
                forAllChildren(node, function (child) {
                    if (typeof child == "string") {
                        ...
                        for (var t = 0; t < tokens.length; t++) {
                            ...
                            if (str.match(/^\s*\n\s*$/)) {
                                str = str.replace(/^\s*$/, '_RET');
                            } else if (str.match(/^\s*$/)) {
                                str = str.replace(/^\s*$/, '_SPACE');
                            } else if (str.match(REGEX_LETTERS_NUMBERS)) {
                                str = str.trim();
                            ...
                            var tokenObj = [str, getTagName(node)];
                            if (section) {
                                tokenObj[2] = section;
                            }
                        ...
                        tokenObjs.push.apply(tokenObjs, recursivelyTokenize(child, section));
                    }
                });
                tokenObjs.push(copyWithValOnChildIndex(node, CLOSE_TAG));
            } else {
                tokenObjs.push(node);
            }
            return tokenObjs;
