...
class MetadataStripper
  def initialize(xml, ecli)
    @xml = xml
    @ecli = ecli
    @about = "#{RECHTSPRAAK_DEEPLINK_ROOT}/uitspraak?id=#{ecli}"
  ...
  def handle_register_metadata(register_metadata)
    ...
    # LJN number
    handle_metadata(register_metadata, @about, 'dcterms:replaces')
    # If the current ECLI is not valid, this points to a replacement ECLI. Note it's only about the identifier.
    handle_metadata(register_metadata, @about, 'dcterms:isReplacedBy')
    # URI for the court
    handle_creator(register_metadata, @about)
    # Judge
    handle_metadata(register_metadata, @about, 'dcterms:contributor')
    # date of judgment
    handle_metadata(register_metadata, @about, 'dcterms:date')

    # Add aliases
    # NOTE: @rdf:language fixed to NL is ignored
    handle_metadata(register_metadata, @about, 'dcterms:alternative')

    # 'Uitspraak' or 'Conclusie'
    handle_case_type(register_metadata, @about)
    # Ex: <psi:procedure rdf:language="nl"
    #      rdfs:label="Procedure"
    #      resourceIdentifier="http://psi.rechtspraak.nl/procedure#eersteAanlegMeervoudig">
    #       Eerste aanleg - meervoudig
    #     </psi:procedure>
    handle_metadata(register_metadata, @about, 'psi:procedure')
    ...
    handle_references(register_metadata, @about)
    handle_coverage(register_metadata, @about) # Jurisdiction
    # Where versions of this judgment can be found. Might be different expressions (e.g., edited and annotated)
    handle_has_version(register_metadata, @about)
    # Relations to other cases
    handle_relations(register_metadata, @about)
    # Existing case numbers
    handle_case_numbers(register_metadata, @about)
    # What kind of law this case is about (e.g., 'staatsrecht)
    handle_subject(register_metadata, @about)
  end
  ...
  def handle_doc_metadata(text_document_metadata)
    ...
    handle_metadata(text_document_metadata, @about, 'dcterms:accessRights')
    # Document modified
    handle_metadata(text_document_metadata, @about, 'dcterms:modified')
    # Document publication date in YYYY-MM-DD
    handle_metadata(text_document_metadata, @about, 'dcterms:issued')
    handle_metadata(text_document_metadata, @about, 'dcterms:publisher') # Publisher
    handle_metadata(text_document_metadata, @about, 'dcterms:title') # Document title

    # Document language; already handled
    # handle_metadata(ml_converter, expression_uri, text_document_metadata, 'dcterms:language')

    # Short summary
    handle_abstract(text_document_metadata, @about) # TODO add lang
  end
